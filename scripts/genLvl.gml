...
while(total > 0)
{
    ...
    {
        objects[total - 1, 2] = obj_enemy_slime;
        monsters--;
    }
    total = monsters + loot + traps + player_spawn + player_exit;
}
...
for(object_current = 0; object_current < array_height_2d(objects); object_current++)
{
    ...
    else if(objects[object_current, 2] == obj_enemy_slime)
    {
        global.room_final[objects[object_current, 0] - x_min, objects[object_current, 1] - y_min] = 4;
    }
    else if(objects[object_current, 2] == obj_raven)
    {
...
for(x_pos = 0; x_pos <= x_max - x_min; x_pos++)
{
    for(y_pos = 0; y_pos <= y_max - y_min; y_pos++)
    {
        ...
        {
            room_instance_add(rm_next, x_pos * 32, y_pos * 32, obj_enemy_slime);
        }
        else if(global.room_final[x_pos, y_pos] == 5)
        {
            room_instance_add(rm_next, x_pos * 32, y_pos * 32, obj_raven);
            player_x = x_pos;
            player_y = y_pos;
